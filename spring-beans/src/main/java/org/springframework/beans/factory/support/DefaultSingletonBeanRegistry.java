...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	/** Cache of singleton objects: bean name --> bean instance 用于保存beanName和创建bean实例之间的关系*/
	private final Map<String, Object> singletonObjects = new ConcurrentHashMap<>(256);

	/** Cache of singleton factories: bean name --> ObjectFactory用于保存beanName和创建bean的工厂之间的关系 */
	private final Map<String, ObjectFactory<?>> singletonFactories = new HashMap<>(16);

	/** Cache of early singleton objects: bean name --> bean instance
	 * 与singletonObjects不同之处：当一个单例bean被放到这里面，那么当bean还在创建过程中，就可以通过getBean方法取到了
	 * 其目的是用来检测循环引用
	 * */
	...
	/**
	 * 直接尝试从缓存获取或者singletonFactories中获取bean
